...
def split_label_free_file(file_name, folder_name, split_number, label_idx):
  data_type = folder_name
  
  split_number
  line_number = get_line_numbers(data_type)[label_idx]
  
  with open(file_name, 'r') as input:
    line_count = 0
    for folder_idx in xrange(split_number):
      end_line_idx = (folder_idx+1)*line_number / split_number
      ...
      while line_count < end_line_idx:
        line = input.readline()
        inst_output.write(line)
        line_count += 1
      inst_output.close()
  
def split(folder_name, split_number):
  ...
  for label_idx, file_name in enumerate(label_free_file_names):
    split_label_free_file(file_name, folder_name, split_number, label_idx)
  
if __name__ == '__main__':
  start = time.time()
  test_folder_name = 'test'
  test_split_number = 2
  split(test_folder_name, test_split_number)
